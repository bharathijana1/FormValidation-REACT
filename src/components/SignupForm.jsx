...
import { ValidationSchemaSignupForm } from './ValidationFormSchema';
import { useForm } from 'react-hook-form';
import { yupResolver } from '@hookform/resolvers/yup';
import LableIndex from './LabelIndex';
import { SignupFormObject } from './FormObject';
...
export default function SignupForm( { title } ) {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm({
    resolver: yupResolver(ValidationSchemaSignupForm),
  });
    ...
    <div className='flex justify-center items-center flex-col h-dvh	'>
      ...
      <div className="shadow-md sm:shadow-orange-300  md:w-3/4 xl:w-3/5 mx-auto md:p-3 ">
        <div className="flex justify-center items-center mx-auto">
          ...
          >
            {
              SignupFormObject.map((item, index) => (
                <LableIndex 
                  key={index}
            ...
            <Link to={"/"} className='px-3 underline text-blue-700'>Login</Link>
            </div>
